...
注意：
+ 如果没有特殊指明，本文档的所有操作**均在 zhangjun-k8s01 节点上执行**，然后远程分发文件和执行命令。
+ nginx代理只是给kubelet,kube-controller-manager,scheduler提供访问的一个入口，您可以以任何TCP负载均衡工具来代替，并不需要一定要用nginx，如果你并不想给kube-apiserver的访问入口做高可用，也可以省略这一节，直接使用kube-apiserver的单个实例作为访问入口
