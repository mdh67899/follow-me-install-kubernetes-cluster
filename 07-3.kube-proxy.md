...
```
注意: 如果您看到以下日志请不要担心,这个是因为node机器上iptables版本比较低，iptables 1.6版本已经添加了对`--random-fully的支持，实际上并不影响您的正常使用，建议使用最新的centos版本。
``` bash
Nov 15 11:01:38 localhost kube-proxy[48253]: I1115 11:01:38.891632   48253 proxier.go:1729] Not using `--random-fully` in the MASQUERADE rule for iptables because the local version of iptables does not support it
```
