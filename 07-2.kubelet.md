...
## 手动 approve server cert csr(必须)

+ 这一步是必须做的，每一个新node加入集群的时候都必须执行这个操作，否则您使用kubectl exec -ti pod_name bash尝试进去容器的时候会有问题，建议您写成脚本，在master上每分钟执行一次

``` bash
[root@localhost ~]# crontab -l
#auto approve kubelet certificate
* * * * * /bin/bash /root/sh/auto_approve_kubelet_certificate.sh
[root@localhost ~]#
[root@localhost ~]# cat /root/sh/auto_approve_kubelet_certificate.sh
#!/bin/bash
